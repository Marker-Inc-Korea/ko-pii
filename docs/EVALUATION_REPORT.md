...
> [!CAUTION]
> **이 보고서는 이전 방법론(2026-05)의 이력 문서이며 운영 승인 근거가 아닙니다.**
> 최신 헤드라인 — 검증 생성셋 540(ko-pii F1 **0.790**),
> 확장셋 1,938(견고성, 0.825) — 은
> **[docs/BENCHMARK.md](BENCHMARK.md)** 를 참조하세요. 아래 표의 행정문서 주입(0.901)·LLM생성-187(0.770)·
> KLUE 는 *다른 방법론*의 이전 수치라 현행과 직접 비교할 수 없고, “운영 가능” 또는
> 고객 SLA의 근거로 인용할 수 없습니다. 현재 도메인 승격 기준은
> [`domain_fit_report.md`](domain_fit_report.md)를 따릅니다.
...
**1. 외부 사람 라벨 데이터 2종과 프로젝트 구축 생성셋**

KDPII와 KLUE-NER는 ko-pii와 독립적으로 만들어진 외부 사람 라벨 데이터다. LLM 생성
벤치마크는 생성 단계에서 ko-pii 룰을 참조하지 않았지만 프로젝트가 구축한 데이터이므로
독립 제3자 검증으로 분류하지 않는다:
...
| **LLM 생성 벤치마크 (187)** ★ | 프로젝트 구축, 생성 시 ko-pii 룰 미참조 | 평가 설계·검토 | LLM 생성 + 동시 라벨 | **0.770** |
...
- **베이스 텍스트** = **AI Hub 569 행정문서 실제 본문** (한국지능정보사회진흥원 공공 데이터). ko-pii 룰 설계에는 사용하지 않았으며, 비교 모델의 사전학습 노출 여부는 확인하지 못했다.
...
- **A. 사용자 자체 라벨링 30~100 문서** — 실제 공문서의 초기 smoke/calibration 표본. 명백한 통합 오류를 찾는 데 유용하지만 통계적 qualification을 대신하지 않음 (작업량: 4~8시간)
...
본 라이브러리는 30~100 문서로 초기 오류를 찾은 뒤, 위험도와 라벨 분포에 맞춘 더 큰
frozen set에서 라벨별 재현율과 신뢰구간을 확인하고 shadow/canary를 거칠 것을 권장한다.

#### 실데이터 PII 주입 코퍼스 — 상세

기존 합성 코퍼스는 13 템플릿 정형이라 양쪽 검출기가 anchor 패턴에 *과적합* 가능. 본 코퍼스는
**AI Hub 569 행정문서 (실제 한국 공공 텍스트, ko-pii 룰 설계에는 미사용)** 의 본문 200 문단을
base로 하고, ko-pii로 사전-redaction 한 뒤 합성 PII를 10가지 anchor 패턴(필드 라벨/서명/괄호/조사 등)으로 주입했다. 비교 모델의 사전학습 노출 여부는 확인하지 못했다. 합성으로 주입한 gold span 위치는 생성 과정에서 결정된다.
...
- 기본 사전 정제: `data/classify_fp_candidates.py` + `dictionaries/common_words.py` (현재 ~300 단어).
  고객별 검토 결과는 패키지 파일을 수정하지 않고 `person_exclusions`로 주입한다.
...
| Tier | F1 | 이력 데이터 내 해석 |
|---|---|---|
| S | ≥0.95 | 이 데이터 분포에서 강한 신호 |
| A | 0.80-0.95 | 추가 도메인 검증 필요 |
| B | 0.50-0.80 | 단독 enforce 근거 부족 |
...
|---|---:|---|
| **행정문서 + PII 주입** (폐기된 자기 주입 방법론) | **0.795** | **운영 근거 사용 금지** |
