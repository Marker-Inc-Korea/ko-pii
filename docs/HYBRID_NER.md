...
→ PII 전용 MoE(openai/PF 1.4B, 0.633) 실측으로, **한국어 양방향 인코더의 우위는 "더 새롭고
큰 모델"로 뒤집히는 문제가 아님**이 확인됐다.
...
- **백본 교체로 풀릴 문제가 아니다** — PII 전용 MoE(openai/PF 0.633)도 klue(0.950)에 크게 못
  미침. 현재 병목은 백본이 아니라 **데이터(도메인 커버리지·gold 품질)**.
