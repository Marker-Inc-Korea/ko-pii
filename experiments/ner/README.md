...
| 17 | **외부 검증 (OOD 2셋 × 6구성)** | 룰·ML 2종·하이브리드 2종·3안 | `external-eval.log` / `external-eval.json` |

**외부 검증 (#17)**: Set A = OOD 주입 v2(민원 도메인·**체크섬 유효 ID**·신규 anchor 8종,
빌더 `data/build_external_eval_v2.py`) + Set B = KLUE-NER dev(인간 라벨, PERSON).
**하이브리드 0.968 > ML단독 0.929 > 룰 0.892** — 인분포에서 뒤집혔던 순위가 외부에서
원복(인분포 ML 우위 = 가짜 gold 아티팩트 확증). 룰 ID recall 4종 전부 1.00 복원.
Set B 에서 ML 퍼지 0.83~0.84 (룰 0.38) — 퍼지 일반화는 진짜. 상세: HYBRID_NER.md 외부 검증 절.
...
```bash
# 의존성 설치 (실측 스택: code/requirements.txt)
