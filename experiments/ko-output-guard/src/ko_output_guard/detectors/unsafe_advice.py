...
_SAFE_CTX = re.compile(
    ...
    r"위험|독성|유독|중독|치명|사망|실명|중태|혼수|손상|해롭|안\s*[되돼됩]|안\s*된|"
    r"절대\s*[안섭금]|피하|삼가|하면\s*안|주의|금물|조심|경고"
)
...
def scan_unsafe_advice(text: str) -> list[Violation]:
    ...
    for pat, sev, code in _PATTERNS:
        for m in pat.finditer(text):
            if _SAFE_CTX.search(_sentence_span(text, m.start(), m.end())):
                continue  # 같은 문장에 안전 경고 → 위험 권장 아님
            out.append(
                Violation(
                    code=code,
